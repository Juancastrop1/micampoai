...
type EventoForm = {
  tipo: string; descripcion: string; fecha: string; costo: string; proxima_fecha: string
}
...
export default function AnimalDetailPage({ params }: { params: { id: string } }) {
  ...
  const [eventoForm, setEventoForm] = useState<EventoForm>({
    tipo: '', descripcion: '', fecha: today, costo: '', proxima_fecha: '',
  })
  ...
  async function handleAddEvento(e: React.FormEvent) {
    ...
    setEventoForm({ tipo: '', descripcion: '', fecha: today, costo: '', proxima_fecha: '' })
    const supabase2 = createClient()
    const { data } = await supabase2.from('eventos_animal').select('*').eq('animal_id', animal.id).order('fecha', { ascending: false })
  ...
  return (
    <div style={{ padding: '32px', maxWidth: '720px' }}>
        ...
        <form onSubmit={handleSave} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
            ...
            <div>
              ...
              <input value={nombre} onChange={e => setNombre(e.target.value)} placeholder="Nombre del animal" style={inputStyle} />
            </div>
            <div>
              <label style={labelStyle}>Raza</label>
              <input value={raza} onChange={e => setRaza(e.target.value)} placeholder="Holstein" style={inputStyle} />
            </div>
            <div>
            ...
            <div>
              ...
              <input type="number" step="0.1" value={peso} onChange={e => setPeso(e.target.value)} placeholder="350" style={inputStyle} />
            </div>
          </div>
          <div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
            ...
            {saved && (
              <span style={{ fontFamily: 'var(--font-sans), sans-serif', fontSize: '13px', color: '#1a6b45' }}>
                ✓ Cambios guardados
              </span>
            )}
          </div>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontFamily: 'var(--font-sans), sans-serif', fontSize: '14px' }}>
              <thead>
                <tr style={{ borderBottom: '1px solid rgba(60,45,30,0.10)' }}>
                  {['Fecha', 'Tipo', 'Descripción', 'Costo', 'Próxima fecha'].map(col => (
                ...
                {eventos.map((ev, i) => (
                  <tr key={ev.id} style={{ borderBottom: i < eventos.length - 1 ? '1px solid rgba(60,45,30,0.07)' : 'none' }}>
                    ...
                    <td style={{ padding: '12px 16px' }}>
                      <span style={{ ...tipoBadge[ev.tipo], padding: '3px 10px', borderRadius: '20px', fontSize: '12px', fontWeight: 500 }}>
                        {tipoLabels[ev.tipo]}
                      </span>
                    </td>
      ...
      {/* Add event modal */}
      {showEventoModal && (
        <div onClick={() => setShowEventoModal(false)}
          ...
          <div onClick={e => e.stopPropagation()}
            ...
            <h2 style={{ fontFamily: 'var(--font-serif), serif', fontWeight: 'normal', fontSize: '22px', color: '#1c1a17', margin: '0 0 22px' }}>Agregar evento</h2>
            <form onSubmit={handleAddEvento} style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '14px' }}>
                <div>
                  <label style={labelStyle}>Tipo *</label>
                  <select value={eventoForm.tipo} onChange={e => setEventoForm(f => ({ ...f, tipo: e.target.value }))} required
                ...
                <div>
                  ...
                  <input type="date" value={eventoForm.fecha} onChange={e => setEventoForm(f => ({ ...f, fecha: e.target.value }))} required style={inputStyle} />
                </div>
              </div>
              <div>
                <label style={labelStyle}>Descripción *</label>
                <input value={eventoForm.descripcion} onChange={e => setEventoForm(f => ({ ...f, descripcion: e.target.value }))} placeholder="Describe el evento" required style={inputStyle} />
              </div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '14px' }}>
                <div>
                  <label style={labelStyle}>Costo (COP)</label>
                  <input type="number" step="0.01" value={eventoForm.costo} onChange={e => setEventoForm(f => ({ ...f, costo: e.target.value }))} placeholder="0" style={inputStyle} />
                </div>
                <div>
                  <label style={labelStyle}>Próxima fecha</label>
                  <input type="date" value={eventoForm.proxima_fecha} onChange={e => setEventoForm(f => ({ ...f, proxima_fecha: e.target.value }))} style={inputStyle} />
                </div>
              </div>
